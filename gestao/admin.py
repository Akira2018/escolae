...
class LivrosAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            return qs
        if request.user.groups.filter(name='Gerentes').exists():
            return qs.filter(escola=request.user.escola)  # Gerentes podem ver os livros da própria escola
        if request.user.groups.filter(name='Alunos').exists():
            return qs.filter(escola=request.user.escola)  # Alunos veem apenas os livros da própria escola
        return qs.none()

    def has_module_permission(self, request):
        if request.user.is_superuser:
            return False  # Superusuários veem o menu
        if request.user.groups.filter(name='Gerentes').exists():
            return False  # Gerentes também veem o menu
        if request.user.groups.filter(name='Alunos').exists():
            return False  # Alunos não veem o menu
        return False  # Outros grupos não têm permissão


class VideosAdmin(admin.ModelAdmin):
    list_display = ('nome_video', 'ano_publicacao', 'colecao', 'qtvideos', 'estante', 'get_nome_escola')
    search_fields = ('nome_video', 'colecao', 'escola__nome_escola')
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            return qs
        if request.user.groups.filter(name='Gerentes').exists():
            return qs.filter(escola=request.user.escola)  # Gerentes veem vídeos da própria escola
        if request.user.groups.filter(name='Alunos').exists():
            return qs.filter(escola=request.user.escola)  # Alunos veem apenas vídeos da própria escola
        return qs.none()
...
class ClientesAdmin(BaseEscolaAdmin):  # Agora herda de BaseEscolaAdmin
    list_display = ('nome_cliente', 'email', 'documento', 'nr_telefone', 'tipo_usuario','get_nome_escola')
    search_fields = ('nome_cliente', 'email', 'documento', 'nr_telefone','nome_escola')
    list_filter = ('tipo_usuario', 'nome_cliente')
    form = ClienteForm
